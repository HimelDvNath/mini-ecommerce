...
const removeFromCart = async (userId: number, productId: number) => {
    ...
    if (result.rows.length === 0) throw new Error("Product not in cart");
...
const placeOrder = async (cartId: number) => {
    ...
    try {
        ...
        const userRes = await client.query(
            `SELECT user_id FROM carts WHERE id = $1 FOR UPDATE`,
            [cartId],
        );

        if (userRes.rows.length === 0) throw new Error("Cart not found");
